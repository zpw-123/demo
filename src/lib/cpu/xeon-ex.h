...
#ifdef SUPPORT_BROADWELL
#include "broadwell_papi.h"
#endif 
#else
...
// 这里的全局变量注册了所能取得的对应微架构 CPU 的事件，绑定了内存模块寄存器
...
cpu_model_t cpu_model_intel_xeon_ex_v4 = {
    .microarch = BroadwellXeon,
#ifdef PAPI_SUPPORT
    .pmc_events = {broadwell_native_events, broadwell_read_stall_events_local, broadwell_read_stall_events_remote},
#else
    .pmc_events = PMC_EVENTS_PTR(broadwell),
#endif
    .set_throttle_register = intel_xeon_ex_set_throttle_register,
    .get_throttle_register = intel_xeon_ex_get_throttle_register
};
