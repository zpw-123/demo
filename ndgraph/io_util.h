...
pair_uint *get_edges_from_file_adj_sym(std::string filename,
  ...
  /*
  if (len != n + m + 2) {
    std::cout << "Bad input file: length = "<<len<< " n+m+2 = " << len << std::endl;
    exit(-1);
  }
  */
...
pair_uint *get_edges_from_file(std::string filename,
                              uint64_t *edge_count, uint32_t *node_count, [[maybe_unused]] bool print = true) {
  //这个地方直接读取边表
  long length;
  std::ifstream file (filename, std::ios::in | std::ios::binary | std::ios::ate);
  ...
  uint64_t *nn=edge_count;
  pair_uint *edges_array = (pair_uint *)malloc((*nn) * sizeof(pair_uint));
  uint64_t start1=0;
  while(start1<(*nn)){
